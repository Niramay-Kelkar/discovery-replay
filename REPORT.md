...
The core system is five processes that hand off through typed artifacts and files, not live calls. Two run as servers: target_app (Flask, port 5001, the hostile legacy UI being automated) and operator_console (Flask, port 5002, the escalation handoff surface). Three run as short-lived CLIs, each owning its own Playwright browser directly: agent.discover, agent.compile_cli, and agent.replay. A sixth, agent.capability_api (Flask, port 5003), is a stretch-goal addition covered in Section 7: a thin HTTP wrapper around the same Replayer the CLI already drives, adding no new execution logic of its own.
...
Stretch goals, per Section 8's own "pick at most one or two" framing: two were built. The agent-facing capability interface (agent.capability_api, Flask on port 5003) wraps the same Replayer the CLI already drives in a thin HTTP layer: a GET /capabilities discovery listing trimmed to what an external caller needs (id, version, description, risk class, confirmation requirement, typed inputs/outputs, deliberately excluding steps, guardrails, and discovery provenance), and a POST /capabilities/<id>/invoke endpoint that maps the existing four-way result contract onto HTTP status codes, Success and BusinessOutcome both 200, HardFailure 422, PendingEscalation 202, with handoff always disabled so a blocking HTTP request can never hang for minutes on a human operator; an escalate trigger still opens in the same SessionStore the operator console reads. Real request/response pairs from a live run are captured in evidence/capability_api/. Multi-run stability (agent.replay_cli --repeat N) replays the same capability and inputs N times sequentially, no parallelism, no retry-on-failure inside the loop, and aggregates a success rate (Success and BusinessOutcome both count as legitimate completions), a failure-determinism verdict computed over any hard failures and grouped by (at_step_id, trigger), and a duration spread, so an artifact's reliability is measured rather than assumed from one pass. A real five-run pass against the live stack is captured in evidence/stability/. Canonicalization/cross-tenant reuse and a second real target variant were judged not worth the build time relative to hardening the single-tenant flow this report is about. Code generation was not pursued at all, since it adds a deliverable with no bearing on any evaluated criterion.

Assisted fallback, bounded, policy-checked LLM recovery invoked only after a step's automatic retries under escalation_policy.max_retries_per_step are exhausted, was deliberately not built as a third stretch goal. Section 8 caps stretch goals at one or two and warns against feature breadth for its own sake; the two built here were chosen because each reinforces a section the evaluation already weights heavily, system design and generalization respectively, rather than adding scope for novelty. If built, it would hook in at exactly the _dispatch_trigger decision point that today maps a trigger to retry, escalate, or fail: a new escalation_policy action, assist, would let a single LLM call propose one alternative locator or action for the current step only, logged as evidence and requiring the same policy-author sign-off every other guardrail already requires, never a second silent attempt beyond the one bounded call. The distinction from max_retries_per_step is the thing that would make this safe rather than a backdoor around Section 3's "no LLM in the replay decision loop" property: automatic retries re-run the identical SETTLE/CHECK against an unchanged plan, where this would let the plan itself flex by exactly one step before falling through to the existing HardFailure/escalate path, and it would still never re-invoke ACT, preserving the same no-double-submit guarantee the rest of the system relies on.
