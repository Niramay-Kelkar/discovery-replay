...
Offline: no browser, no API. Uses the real Phase 3 trajectory on disk
plus a couple of hand-built ones for the failure paths.
"""
import json
...
REAL_TRAJ = (
    ...
    / "evidence/runs/disc-20260909-182538/trajectory.json"
)
...
def _minimal_policy(**overrides) -> PolicySpec:
    base = dict(
        ...
        output_name_mapping={"member_name": "full_name",
...
def test_compiles_the_real_trajectory():
    ...
    # 5 trajectory action steps (type, click, click, extract, extract) + 1
    # synthesized entry navigation; `done` is dropped.
    assert [s.action for s in cap.steps] == [
        "navigate", "fill", "click", "click", "extract", "extract"
    ]
    assert cap.steps[0].id == "open_entry"
    assert cap.policy_authored_by
    assert cap.discovery.run_id == "disc-20260909-182538"
...
def test_fill_value_is_parameterized_not_hardcoded():
    ...
    # the literal "M1001" appears in no step locator or value template
    # (the InputParam.example and human-readable descriptions may still
    # carry it -- those are not what replay resolves against).
    for s in cap.steps:
        assert "M1001" not in (s.value_template or "")
        for loc in s.locators:
            ...
            assert "M1001" not in json.dumps(resolvable)
...
def test_extract_locators_come_from_label_not_value():
    ...
    assert "$18,750.42" not in cap.to_json()


def test_link_name_with_member_data_is_generalized_to_a_substring():
    cap, notes = compile_capability(_real_trajectory(), POLICY_SPEC, INPUT_BINDINGS)
    link = cap.steps[3]  # click open detail
...
def test_allowlist_routes_narrowed_to_visited():
    cap, _ = compile_capability(_real_trajectory(), POLICY_SPEC, INPUT_BINDINGS)
    assert cap.guardrails.allowlist_routes == ["/", "/member/*", "/search"]
...
def test_output_name_mapping_normalizes_to_the_contract_name():
    cap, notes = compile_capability(_real_trajectory(), POLICY_SPEC, INPUT_BINDINGS)
    # discovery captured "member_name"; the contract name is "full_name"
    assert [o.name for o in cap.outputs] == [
        "full_name", "savings_balance", "outcome_code"
    ]
    assert "member_name" not in cap.to_json()
    name_step = next(s for s in cap.steps if s.output_name == "full_name")
    assert name_step.id == "extract_full_name"
    # terminal checkpoint + step ids all use the normalized name
    term = cap.steps[-1].checkpoint
    onc = next(c for c in term.checks if c.kind == "outputs_non_empty")
    assert set(onc.outputs) == {"full_name", "savings_balance"}
...
def test_matches_example_artifact_output_naming():
    cap, _ = compile_capability(_real_trajectory(), POLICY_SPEC, INPUT_BINDINGS)
    example = json.loads(
        (Path(__file__).resolve().parents[2] / "schema/example_artifact.json").read_text()
    )
    example_names = {o["name"] for o in example["outputs"]}
    compiled_names = {o.name for o in cap.outputs}
    ...
    assert compiled_names <= example_names
    assert {"full_name", "savings_balance", "outcome_code"} <= compiled_names
...
def test_identity_mapping_is_allowed():
    # a policy that keeps every discovered name still has to say so
    policy = _minimal_policy(
        capability_id="member_lookup",
        output_name_mapping={"member_name": "member_name",
                             "savings_balance": "savings_balance"},
    )
    cap, _ = compile_capability(_real_trajectory(), policy, INPUT_BINDINGS)
    assert [o.name for o in cap.outputs] == [
        "member_name", "savings_balance", "outcome_code"
    ]


def test_search_field_gap_is_surfaced_not_papered_over():
    cap, notes = compile_capability(_real_trajectory(), POLICY_SPEC, INPUT_BINDINGS)
    assert [i.name for i in cap.inputs] == ["search_term"]  # no search_field
    assert not any(s.action == "click" and "radio" in s.description.lower()
                   for s in cap.steps)
    assert any("search_field" in g for g in notes.gaps)
...
def test_unbound_fill_literal_is_recorded_as_a_gap():
    ...
    fill = next(s for s in cap.steps if s.action == "fill")
    assert fill.value_template == "M1001"  # left literal, not invented
